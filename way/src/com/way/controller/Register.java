...
public class Register {
	...
	public Object checkCode(HttpSession session,String code){
		...
		if(!code.equals("") && code!=null){
			if(code.equalsIgnoreCase(checkcode)){	//验证码不正确，返回注册页面
				json.put("valid", true);
			}else{
	...
	public String register(User user,HttpSession session,String checkcode,
			@RequestParam("imgefile")MultipartFile imgefile,HttpServletRequest request){
		try{
			//数据校验
			Validate.notEmpty(user.getUserName(),"用户名为空");
			Validate.notEmpty(user.getTelephone(),"电话号码为空");
			Validate.notEmpty(user.getPassword(),"密码为空");
			Validate.notEmpty(user.getMail(),"邮箱为空");
			Validate.notEmpty(user.getRole(),"角色为空");
		
			String salt = String.valueOf(new Date().getTime());//设置盐值
			String newPassword = PasswordEncoder.getNewPassword(salt, user.getPassword());//通过hash算法，算出新密码
			user.setSalt(salt);//设置盐值
			user.setPassword(newPassword);//设置密码
			user.setAfalse("0");
			user.setImg("head.png");
			if(user.getRole().equals("0")){//如果身份证号码为空，用户状态为1（启用）表示是游客
				user.setState("1");
			}else{
				user.setState("0");
			...
			if(user.getRole().equals("0")){//是游客,直接跳转到首页，不需要登录
	...
	public void checkImg(HttpServletRequest request,HttpServletResponse response)
		...
		// 将生成的字母存入到session中
//		request.setAttribute("checkcode", sb.toString());
